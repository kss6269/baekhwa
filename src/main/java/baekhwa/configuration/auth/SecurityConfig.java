	...
	protected void configure(HttpSecurity http) throws Exception {
		...
		http
			...
			.and()
				...
				.anyRequest().authenticated()//설정된 이외 나머지url들 인증된 사용자만 허용(로그인한 사용자)
			.and()
				.logout()
					.logoutSuccessUrl("/")//로그아웃시 "/" 주소로 이동하세요
			.and()
				.oauth2Login()//oauth2로그인 기능 설정
					.userInfoEndpoint()//oauth2 로그인 성공이후 사용자 정보를 가져올때의  설정
						.userService(oAuth2UserService);//로그인 성공시 진행 할 구현체 등록
